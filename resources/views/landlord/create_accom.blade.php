                    ...
                    <div class="landlord-info">
                        <form method="POST" action="{{ route('landlord.store') }}" enctype="multipart/form-data">
                            ...
                            <div class="form-check form-check-inline">
                                <input class="form-check-input @error('accomodation') is-invalid @enderror" type="radio" name="accomodation"
                                    value="{{ old('accomodation', 'single') }}">
                                <label class="form-check-label">Je possede un seul logement</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input @error('accomodation') is-invalid @enderror" type="radio" name="accomodation"
                                    value="{{ old('accomodation', 'many') }}">
                                <label class="form-check-label">Je possede plusieurs logements</label>
                            </div><br>
                            @error("accomodation") <span class="invalid-feedback">{{ $message }}</span> @enderror

                            <div class="mt-3">
                                <p class="mt-5">Description?</p>
                                <textarea class="form-control @error('description') is-invalid @enderror" cols="30" rows="10" name="description" placeholder="Entrez une description du/des logement(s)">{{ old('description') }}</textarea>
                            ...
                            <div class="mt-5 d-lg-flex align-items-center justify-content-between">
                                ...
                                @error("number_of_rooms") <span class="invalid-feedback">{{ $message }}</span> @enderror
                            </div>

                            <p class="mt-5">Recherchez-vous un locataire?</p>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="need_tenant" value="{{ old('need_tenant', 'yes') }}">
                                <label class="form-check-label">Oui je recherche un locataire</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="need_tenant" value="{{ old('need_tenant', 'no') }}">
                                <label class="form-check-label">Non j'ai deja un locataire</label>
                            </div><br>
                            @error("need_tenant") <span class="invalid-feedback">{{ $message }}</span> @enderror

                            <div class="mt-5 d-lg-flex align-items-center justify-content-between">
                                <p class="mb-3 mb-lg-0">Le logement serait il louer meuble?</p>
                                <div>
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="furnished" value="{{ old('furnished', 'yes') }}">
                                        <label class="form-check-label">Oui</label>
                                    </div>
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="furnished" value="{{ old('furnished', 'no') }}">
                                        <label class="form-check-label">Non</label>
                                    </div>
                                </div>
                                @error("furnished") <span class="invalid-feedback">{{ $message }}</span> @enderror
                            </div>

                            <div class="mt-5 d-lg-flex align-items-center justify-content-between">
                                <p class="mb-3 mb-lg-0">Superficie approximative du logement</p>
                                <input type="text" class="@error('approx_surface_area') is-invalid @enderror" name="approx_surface_area" value="{{ old('approx_surface_area') }}">
                                @error("approx_surface_area") <span class="invalid-feedback">{{ $message }}</span> @enderror
                            </div>

                            <div class="mt-5 d-lg-flex align-items-center justify-content-between">
                                <p class="mb-3 mb-lg-0">Montant approximatif du loyer</p>
                                <input type="text" class="@error('monthly_rent_price') is-invalid @enderror" name="monthly_rent_price" value="{{ old('monthly_rent_price') }}">
                                @error("monthly_rent_price") <span class="invalid-feedback">{{ $message }}</span> @enderror
                            </div>

                            <div class="mt-5 d-lg-flex align-items-center justify-content-between">
                                <p class="mb-3 mb-lg-0">Quand etes vous disponible</p>
                                <input type="date" class="@error('availability_date') is-invalid @enderror" name="availability_date" value="{{ old('availability_date') }}">
                                @error("availability_date") <span class="invalid-feedback">{{ $message }}</span> @enderror
                            </div>
